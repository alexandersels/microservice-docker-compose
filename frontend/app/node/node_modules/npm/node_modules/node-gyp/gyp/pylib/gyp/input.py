...
# itself and should not be propagated into its com.test.project.userservice.configurations.  It is merged
# with a list that can come from the generator to
# create non_configuration_keys.
base_non_configuration_keys = [
  # Sections that must exist inside targets and not com.test.project.userservice.configurations.
  'actions',
  'com.test.project.userservice.configurations',
  'copies',
  ...
  # Sections that can be found inside targets or com.test.project.userservice.configurations, but that
  # should not be propagated from targets into their com.test.project.userservice.configurations.
  'variables',
]
...
def MergeConfigWithInheritance(new_configuration_dict, build_file,
  ...
  configuration_dict = target_dict['com.test.project.userservice.configurations'][configuration]
...
def SetUpConfigurations(target, target_dict):
  ...
  # com.test.project.userservice.configurations does not.
  if not 'com.test.project.userservice.configurations' in target_dict:
    target_dict['com.test.project.userservice.configurations'] = {'Default': {}}
  if not 'default_configuration' in target_dict:
    concrete = [i for (i, config) in target_dict['com.test.project.userservice.configurations'].items()
                if not config.get('abstract')]
    target_dict['default_configuration'] = sorted(concrete)[0]

  merged_configurations = {}
  configs = target_dict['com.test.project.userservice.configurations']
  for (configuration, old_configuration_dict) in configs.items():
    # Skip abstract com.test.project.userservice.configurations (saves work only).
  ...
  # Put the new com.test.project.userservice.configurations back into the target dict as a configuration.
  for configuration in merged_configurations.keys():
    target_dict['com.test.project.userservice.configurations'][configuration] = (
        merged_configurations[configuration])

  # Now drop all the abstract ones.
  for configuration in target_dict['com.test.project.userservice.configurations'].keys():
    old_configuration_dict = target_dict['com.test.project.userservice.configurations'][configuration]
    if old_configuration_dict.get('abstract'):
      del target_dict['com.test.project.userservice.configurations'][configuration]

  # Now that all of the target's com.test.project.userservice.configurations have been built, go through
  # the target dict's keys and remove everything that's been moved into a
  # "com.test.project.userservice.configurations" section.
  delete_keys = []
  ...
  # Check the com.test.project.userservice.configurations to see if they contain invalid keys.
  for configuration in target_dict['com.test.project.userservice.configurations'].keys():
    configuration_dict = target_dict['com.test.project.userservice.configurations'][configuration]
...
def Load(build_files, variables, includes, depth, generator_input_info, check,
  ...
  # Move everything that can go into a "com.test.project.userservice.configurations" section into one.
  for target in flat_list:
    target_dict = targets[target]
